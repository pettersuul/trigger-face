...
# Select input
input = 'dataset/average/trigger-average.jpg'
...
try:
    if sys.argv[1]:
        ...
        arg = sys.argv[1]
        print('arg: ' + arg)
        if os.path.isfile(arg) and arg.lower().endswith(('.png', '.jpg', '.jpeg')):
            input = arg
        else:
...
image = cv2.imread(input)  # read any image containing a face
