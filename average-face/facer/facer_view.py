...
def detect_face_landmarks(images,
    ...
    for n, (file, image) in enumerate(images.items()):
        ...
        for num, face in enumerate(found_faces):
            ...
            nose.append(lm[30])
            eyeright = lm[36:42]
            eyeright.append(lm[36])
            eyeleft = lm[42:48]
            eyeleft.append(lm[42])
            lipsouter = lm[48:60]
            lipsouter.append(lm[48])
            lipsinner = lm[60:80]
            lipsinner.append(lm[60])
            lipsweb = [
                (lm[48],lm[60]),
                (lm[60],lm[49]),
                (lm[49],lm[61]),
            ...
            plt.plot([x for x, y in chin], [y for x, y in chin], color="cyan", linewidth=0.5)
            plt.plot([x for x, y in eyebrows], [y for x, y in eyebrows], color="cyan", linewidth=0.5)
            #plt.plot([x for x, y in eyebrowsweb], [y for x, y in eyebrowsweb], color="cyan", linewidth=0.5)
            plt.plot([x for x, y in nose], [y for x, y in nose], color="cyan", linewidth=0.5)
            plt.plot([x for x, y in eyeright], [y for x, y in eyeright], color="cyan", linewidth=0.5)
            plt.plot([x for x, y in eyeleft], [y for x, y in eyeleft], color="cyan", linewidth=0.5)
            plt.plot([x for x, y in lipsouter], [y for x, y in lipsouter], color="cyan", linewidth=0.5)
            plt.plot([x for x, y in lipsinner], [y for x, y in lipsinner], color="cyan", linewidth=0.5)
            #for lines in lipsweb:
            #    plt.plot([x for x, y in lines], [y for x, y in lines], color="cyan", linewidth=0.5)
